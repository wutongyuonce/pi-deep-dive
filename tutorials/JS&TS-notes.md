...
<img src="img/image-20260720202852921.png" alt="image-20260720202852921" style="zoom:50%;" />

它是一个为 TypeScript 设计的 **JSON Schema 类型构建器**，核心价值是：**“一份 Schema 定义，同时驱动运行时验证（AJV）和编译时类型检查（TS）”**。常用在为 Agent 定义 **Tool 的参数 schema**。

* 编译时类型检查（给 TS 编译器看），当你写下 `type User = Static<typeof UserSchema>` 时：

  - TS 编译器会在**你敲代码的那一刻（编译时）**，去“读取”`UserSchema` 的结构。

  - 它知道 `id` 是 `string`，`age` 是可选的 `number`。

  - 此时如果你写 `const a: User = { id: 123 }`，TS 会直接在编辑器中报红线（因为 `id` 类型不对）。**这利用了编译时的静态检查能力。**

* 运行时数据验证（给 JS / AJV 执行），当你的代码真正跑起来，从数据库或 API 拿到数据时：

  - 你把这个 `UserSchema`（此时是纯 JS 对象）传给 **AJV** 验证器。

  - AJV 在**运行时**拿着真实数据 `{ id: "abc", name: "Tom" }`，去比对 `UserSchema` 里的规则（`format: 'uuid'`, `minLength: 2`）。

  - 如果数据不符合规则，AJV 会在运行时抛出错误。**这利用了运行时的动态验证能力。**

1、定义 Schema（替代手写 JSON Schema）
...
// 自动推导 TS 类型
type User = Static<typeof UserSchema>; // { name: string; age: number; email: string; tags: string[] }
```
...
Value.Check(UserSchema, { name: "Alice", age: 25 }); // false（缺 email 和 tags）
const errors = [...Value.Errors(UserSchema, data)];  // 获取详细错误
Value.Convert(UserSchema, data);  // 自动类型转换（字符串 "25" → 数字 25）
```

`Value.Check` / `Value.Errors` 帮你做运行时验证，而 `Value.Convert` 会自动做类型强制转换（类似 JSON Schema 的 coercion）。
...
validator.Check(data);     // 比 Value.Check 更快
[...validator.Errors(data)]; // 错误信息
```

编译后的校验器比 `Value.*` 快得多，适合高频调用。
