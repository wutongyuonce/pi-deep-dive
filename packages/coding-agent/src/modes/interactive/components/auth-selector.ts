...
/**
 * Component that renders a provider selector for stored auth configuration.
 */
export class AuthSelectorComponent extends Container implements Focusable {
	...
	constructor(
		...
		const title = mode === "login" ? "Select provider to configure:" : "Select provider to remove:";
			...
			const message =
				this.allProviders.length === 0
					? this.mode === "login"
						...
						: "No providers configured. Use /login first."
	...
	private formatStatusIndicator(provider: AuthSelectorProvider): string {
		...
		if (credential) {
			return theme.fg("muted", " • ") + theme.fg("warning", "API key configured");
		}
