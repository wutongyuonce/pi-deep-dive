	...
	(async () => {
		...
		try {
			...
			for await (const chunk of openaiStream) {
				...
				// Fallback: some providers return usage
			...
			// Some providers give additional information in this field.
		...
		if (msg.role === "user") {
			...
			if (nonEmptyThinkingBlocks.length > 0) {
				if (compat.requiresThinkingAsText) {
					...
					// objects is non-standard and causes some models to mirror the content-block
					// structure literally in their output, producing recursive nesting.
...
function parseChunkUsage(
	...
	// Follow documented OpenAI semantics: cached_tokens is cache-read
	// tokens (hits). Some providers can include cache_write_tokens as a separate write count.
...
/**
 * Default compatibility settings for standard OpenAI completions APIs.
 ...
 * Uses explicit model.compat if provided, otherwise returns defaults.
 */
