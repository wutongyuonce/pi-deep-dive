...
convertToLlm / prepareNextTurn / transformContext / shouldStopAfterTurn
├─ 场景：核心流程出错
├─ 影响范围：整个对话循环
...
class Agent {
  ...
  private readonly listeners = new Set<...>();     // 事件订阅者
...
Agent 同一时间只能有一个 `activeRun`。`prompt()` / `continue()` 开始时创建，`finishRun()` 结束时清除。它同时承载了三个职责：
...
1. **互斥锁（`if (this.activeRun) throw`）**
   ...
   逻辑：`activeRun` 存在 = 有 run 正在跑 → 新的 `prompt()` / `continue()` 直接报错。`activeRun = undefined` = 空闲 → 可以开始新 run。
...
3. **空闲等待（`waitForIdle()`）**
   ...
   ```json
   // 创建时，同时创建一个 Promise 和它的 resolve 函数
   let resolvePromise = () => {};
   const promise = new Promise<void>((resolve) => { resolvePromise = resolve; });
   this.activeRun = { promise, resolve: resolvePromise, ... };
...
互斥保护：`prompt()` 和 `continue()` 开头都检查 `if (this.activeRun) throw`，同一时间只能有一个 run 在执行。
...
```typescript
get state(): AgentState        // 只读视图，返回 _state（向上转型）
get signal(): AbortSignal | undefined  // 当前 run 的 AbortSignal
```
...
```typescript
steer(message: AgentMessage): void     // 入队 steering 消息
followUp(message: AgentMessage): void  // 入队 follow-up 消息
```
...
```typescript
set steeringMode(mode: QueueMode)   // "one-at-a-time" | "all"
...
- listener 的 Promise 被 await：按注册顺序串行执行，保证状态归约和事件通知的顺序一致
- `agent_end` 不等于 idle：Agent 要等所有 `agent_end` listener 处理完才算空闲

### 核心机制：`runWithLifecycle()`

`prompt()` 和 `continue()` 的本质是同一模式：

```
prompt(input)
  → normalizePromptInput(input) → messages
  → runWithLifecycle(async (signal) => {
      runAgentLoop(messages, createContextSnapshot(), createLoopConfig(), processEvents, signal, streamFn)
    })

continue()
  → runWithLifecycle(async (signal) => {
      runAgentLoopContinue(createContextSnapshot(), createLoopConfig(), processEvents, signal, streamFn)
    })
```
...
**`handleRunFailure()`** — 循环引擎正常运行时会按协议发出完整事件序列（`message_start` → `message_end` → `turn_end` → `agent_end`）。如果引擎内部抛出未捕获异常，事件序列会中断。`handleRunFailure()` 补发一组最小失败事件，确保所有 listener 能收到一致的生命周期信号：
...
**`finishRun()`** — 无论成功还是失败，finally 块都会调用它：
...
| 模型选择 | 只持有一个 `model` 字段 | ModelRegistry |
