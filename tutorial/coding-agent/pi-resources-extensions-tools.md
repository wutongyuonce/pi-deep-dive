...
> **定位**：本篇解释 `coding-agent` 怎样把 settings、packages、extensions、skills、prompts、themes、tools 装进一个统一运行时。
> 前置依赖：建议先看 `tutorial/pi-coding-agent.md` 和 `tutorial/coding-agent/pi-startup-runtime-and-modes.md`。
> 适用场景：当你想知道外部资源是如何被发现、合并、注入 system prompt，并最终影响 agent 行为的。

## 先下一个总判断

`coding-agent` 的资源系统最重要的设计不是“支持很多资源类型”，而是：
...
> 对 `coding-agent` 来说，资源系统不是附属功能，而是运行时本身的一部分。
