...
1、**工具定义的 JSON Schema**：注意 `tools` 列表中每个工具的格式——`name` 是函数名，`description` 告诉模型"什么时候该用这个工具"，`parameters` 描述参数类型和含义。这个 Schema 会随请求一起发送给 LLM，模型正是通过它来理解工具的能力边界

2、**工具描述的质量至关重要**：`description` 字段是模型决策的核心依据。一个好的工具描述应该包含：工具的适用场景（什么时候用）、不适用场景（什么时候不用）、参数的具体含义和格式。描述模糊会导致模型误调用或漏调用——比如把 `send_email` 描述为"处理邮件相关任务"，模型可能在用户只是问"怎么写邮件"时也去调用它

3、`strict: true` **模式**：在工具定义中加入 `"strict": True`，可以启用结构化输出，确保模型生成的参数严格符合 JSON Schema。开启后需要同时设置 `"additionalProperties": False`，并将所有参数都放入 `required`列表。这在生产环境中强烈推荐，可以避免参数缺失或类型错误导致的运行时崩溃

> JSON Schema 既是 AI 的**“说明书”**（告诉它有什么工具），也是 AI 必须遵守的**“法律”**（规定它必须怎么传参）

4、**错误处理**：工具执行应该包裹在 `try/except` 中。当工具调用失败时，不要直接抛出异常中断流程，而是将错误信息以 JSON 格式返回给模型——模型会根据错误信息自行决策，比如换个参数重试、或者告知用户操作失败

5、**执行循环（Agent Loop）**：`run_agent` 函数的核心是一个 `while True` 循环。每次循环中，我们把完整的消息历史（包括之前的工具调用结果）发给 LLM。模型返回后，我们检查 `finish_reason`：如果是 `"stop"`，说明模型已经给出最终回答；如果是 `"tool_calls"`，说明模型需要使用工具。执行完工具后，我们把结果以 `role: "tool"` 的消息追加到历史中，然后继续下一轮循环

6、**工具函数映射表**：`tool_functions` 字典将函数名字符串映射到实际的 Python 函数。这个设计让工具的调用和注册可以解耦——你只需要在字典中添加新的映射，就能让 Agent 获得新的能力
